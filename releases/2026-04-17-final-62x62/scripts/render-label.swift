...
func drawCheckinAppBadge(name: String, department: String, in rect: NSRect, logoPath: String?) {
    ...
    let headerHeight = rect.height * (isSquareBadge ? 0.20 : 0.16)
    ...
    let titleFontSize = min(rect.width * (isSquareBadge ? 0.082 : 0.090), isSquareBadge ? 56 : 64)

    drawTextBlock(
        spec.churchName ?? "ICF TEL AVIV",
        in: NSRect(
            x: rect.width * (isSquareBadge ? 0.03 : 0.08),
            y: rect.maxY - headerHeight + headerHeight * (isSquareBadge ? 0.18 : 0.08),
            width: rect.width * (isSquareBadge ? 0.94 : 0.84),
            height: headerHeight * (isSquareBadge ? 0.58 : 0.82)
        ),
        fontName: "",
    ...
    drawTrackedCenteredText(
        name.uppercased(),
        in: nameRect,
        fontSize: min(rect.width * (isSquareBadge ? 0.24 : 0.20), isSquareBadge ? 168 : 148),
        weight: .bold,
        color: black,
        tracking: max(rect.width * (isSquareBadge ? 0.0015 : 0.014), isSquareBadge ? 1 : 10)
    )

    if !department.isEmpty {
        let departmentY = isSquareBadge
            ? rect.minY + rect.height * 0.20
            : nameRect.minY - rect.height * 0.22
        ...
        drawTrackedCenteredText(
            ...
            in: groupRect,
            fontSize: min(rect.width * (isSquareBadge ? 0.13 : 0.12), isSquareBadge ? 90 : 84),
            weight: .bold,
            color: black,
            tracking: max(rect.width * (isSquareBadge ? 0.001 : 0.010), isSquareBadge ? 0.5 : 6)
        )
    }
}
